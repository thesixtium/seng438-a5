...
In Part 1 (using RGT), we determined a good model for predicting failure growth by comparing best fit measures for the different hazard functions. Using our chosen model, we determine it would take 60 intervals to guarantee reliability at a 0.5 failure intensity. It can be seen that a larger dataset could be used to try and predict the reliability of the SUT.
