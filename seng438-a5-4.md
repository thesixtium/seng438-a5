...
Here we can see that what I judge to be the best model GM will take close to 60 intervals to get a failure intensity down to 0.5 but our next best model NB2 predicts 20 intervals to reach target reliability. These significant variations in model predictions play a big role in estimating software reliability growth, and can make a big difference in investment prioritization for software.
...
In Part 1 (using RGT), we determined a good model for predicting failure growth by comparing best fit measures for the different hazard functions. Using our chosen model, we determine it would take 60 intervals to guarantee reliability at a 0.5 failure intensity. It can be seen that a large dataset could be used to try and predict the reliability change of the SUT over time.
 
In Part 2 (using RDC), we do not try to predict, instead we demonstrate how reliable our system already is to determine if the system is ready (reliable enough to meet user criteria) or if more testing is still required. Both charts reflect reliability but with different aims and uses.
